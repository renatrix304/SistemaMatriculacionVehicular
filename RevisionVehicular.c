...
void validarMatricula(Vehiculo *v) {
		...
		if (strlen(tempAnio) == 4) {
			...
			if (valido) {
				...
				if (anioNumerico < 1940 || anioNumerico > 2025) {
					valido = 0;
					printf("Anio no valido, el límite de anio del vehiculo esta entre 1940 y 2025\n");
				}
			}
		}
		if (!valido) {
	...
	do {
		...
		if (!valido) {
			printf("Cantidad invalida, ingrese un valor de entre 4 a 6 cifras numéricas\n");
		} else {
...
float calcular_matricula_vehicular(const char tipo_vehiculo[], int cilindraje, int anio_fabricacion,
	...
	if (revision_obligatoria == 0) {
		total += 50.0f; // Recargo por no tener revision
	}
